...
outPath = fullfile(rootPath,nets{n},'indiParcellation');
...
%     if exist(fullfile(outPath,[sub(i).name,'.mat']));continue;end
    for j = 1
        load(fullfile(probmapPath,sub(i).name,['region',num2str(j),'_probmap.mat']));
        currParcProb = y_img(voxelInd);
        if isnan(sum(currParcProb))
            troubleSub = [troubleSub;sub(i).name];
        end
    end
    toc
    fprintf('sub %d finished\n',i)
end
end
...
for i = 1:length(nets)
    ...
    ParcelSize(:,60) = [];
    save(fullfile(path,[nets{i},'.mat']),'ParcelSize');
end
load(fullfile(path,'..','testInformation.mat'));
testSubInformation(60,:) = [];
save(fullfile(path,'..','testInformation.mat'),'testSubInformation');