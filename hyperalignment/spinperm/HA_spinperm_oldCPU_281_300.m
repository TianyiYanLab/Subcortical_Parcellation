...
for block_start = 281:block_size:300
    ...
    % ¡¾Performance optimization 3¡¿£ºBefore starting the subject loop, precompute the row-slice indices for all perm and all net combinations!
    ...
    for n_sub = 1:length(sub)
        ...
        if ~exist(h5_file, 'file') 
            warning('File does not exist, skipping: %s', h5_file);
            continue;
        end
