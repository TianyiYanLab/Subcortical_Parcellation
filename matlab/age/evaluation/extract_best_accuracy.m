% Purpose: extract best accuracy.
% Input: load(fullfile(PredictionPath,num2str(i),net,'setup.mat'),'sub_fold');%Extract the subject indices for each folded test set; load([str1,'/',net,'/','final_result.mat'],'y_predict_concat');
% Output: save(fullfile(str1,net,'yOrgAndPredConcat.mat'),'y_org_res','y_predict_concat');
% Dependencies: MATLAB R2019b; see README.md.
%MATLAB cbig
clear;
PredictionPath = repo_path('results/prediction/age/prediction');
