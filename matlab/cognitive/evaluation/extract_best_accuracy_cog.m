% Purpose: extract best accuracy cog.
% Input: load(fullfile(CurrRepPath,'setup.mat'),'sub_fold'); % Extract subject indices for each fold's test set; load(fullfile(CurrRepPath,'final_result.mat'),'y_predict_concat');
% Output: save(fullfile(CurrRepPath,'yOrgAndPredConcat.mat'),'y_org_res','y_predict_concat');
% Dependencies: MATLAB R2019b; see README.md.
%MATLAB cbig
clear;
PredictionPath = repo_path('results/prediction/cognition/prediction');
