% Purpose: gen kfold.
% Input: example = readtable(fullfile('data input',[cogTasks{ti},'.csv']));; load(fullfile('prediction',cogTasks{ti},num2str(rep),'splits.mat'));
% Output: save(fullfile('prediction',cogTasks{ti},num2str(rep),'StratifiedKfolds.mat'),'sub_fold');
% Dependencies: MATLAB R2019b; see README.md.
clear
cd(repo_path('results/prediction/cognition'));
replication = 100;
