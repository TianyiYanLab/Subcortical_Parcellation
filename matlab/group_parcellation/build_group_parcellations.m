% Purpose: build group parcellations.
...
% Output: mat2nii(parcellation,fullfile(out,[labels{indNet},'_',nucleus{indNucleus},'.nii']),size(parcellation),32,fullfile(mask_path,[nucleus{indNucleus},'_LEFT.nii']));
% Dependencies: SpectralClustering_Larry, mat2nii, read
%% Accumulated FCS version
clear
FCS_path = repo_path('results/hyperalignment/forward');
mask_path = repo_path('data/templates/nucleus_masks');
out_path = repo_path('results/group_parcellation');
