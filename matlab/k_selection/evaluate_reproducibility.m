% Purpose: evaluate reproducibility.
% Input: load(repo_path('data/derived/split_half_indices.mat'));; load(fullfile(FCS_path,subList(indSub).name,[nucleus{indNucleus},'.mat']));
% Output: save(fullfile(group_parcellation_out_sym,['it_',num2str(indPerm),'.mat']),'group1_L_parcellation_sym','group1_R_parcellation_sym','group2_L_parcellation_sym','group2_R_parcellation_sym');; save(fullfile(out_path,[nucleus{indNucleus},'_random_group_reproducibility.mat']),'repro_sym');
% Dependencies: SpectralClustering_multiK_Larry
...
load(repo_path('data/derived/split_half_indices.mat'));
out_path = repo_path('results/work/hyperalignment/Ksolution/random group reproducibility');
