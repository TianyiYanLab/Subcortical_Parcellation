...
# Input: df = pd.read_csv(fr'{rootPath}/{t}.csv')
# Output: scipy.io.savemat(
...
rootPath = str(REPO_ROOT / 'results/prediction/cognition/data input')
...
for t in cogTasks:
    ...
    for repeat_idx, split_group in enumerate(repeated_splits):
        currRepPath = str(REPO_ROOT / 'results/prediction/cognition/prediction' / t / str(repeat_idx))
